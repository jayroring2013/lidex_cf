...
type HomeData = {
  ...
  trendingAnime: Cover[]
  trendingManga: Cover[]
  trendingNovels: Cover[]
  typeCounts: number[]
  topAnime: any[]
  recentManga: any[]
  recentNovels: any[]
  votingNovels: CarouselItem[]
  mangaVolCovers: Record<number, string | null>
  novelVolCovers: Record<number, string | null>
}
...
function SafeImg({ src, alt, className }: { src: string; alt: string; className?: string }) {
  ...
  return <img src={src} alt={alt} loading="lazy" decoding="async" className={className ?? 'w-full h-full object-cover block'} onError={() => setErr(true)} />
}
...
function HomeFeatureIcon({ icon: Icon, title, subtitle }: {
  ...
  return (
    <div className="flex flex-col items-center text-center gap-2 min-w-[118px] sm:min-w-[132px]">
      <Icon className="w-7 h-7 sm:w-8 sm:h-8" strokeWidth={1.8} style={{ color: 'var(--foreground)' }} />
      <div>
        <p className="text-[11px] sm:text-xs font-black leading-tight" style={{ color: 'var(--foreground)' }}>{title}</p>
        <p className="text-[9px] sm:text-[10px] mt-0.5 leading-snug" style={{ color: 'var(--foreground-muted)' }}>{subtitle}</p>
      </div>
    </div>
  )
}
...
function HomeFeatureStrip({ vi }: { vi: boolean }) {
  const features = [
    { icon: Info, title: vi ? 'Thông tin' : 'Information', subtitle: vi ? 'Cập nhật LN yêu thích' : 'Up-to-date favourite LN' },
    { icon: BrainCircuit, title: vi ? 'Phân tích sâu' : 'In-depth Analysis', subtitle: vi ? 'Hiểu rõ từng tựa' : 'Title-level insight' },
    { icon: Building2, title: vi ? 'Dữ liệu NPH' : 'Deep-dive Data', subtitle: vi ? 'Nhà phát hành đứng sau LN' : 'Publisher behind the LN' },
    { icon: Sparkles, title: vi ? 'Dự đoán' : 'Prediction', subtitle: vi ? 'Khả năng cấp phép tương lai' : 'Future licensing likelihood' },
    { icon: Trophy, title: vi ? 'Xếp hạng LN' : 'Ranking', subtitle: vi ? 'BXH LN yêu thích' : 'Favourite LN ranking' },
  ]
    ...
    <section className="pt-7 sm:pt-8 pb-4">
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
        ...
        <p className="text-center text-[10px] sm:text-xs mt-6" style={{ color: 'var(--foreground-muted)' }}>
          {vi ? 'Tính năng và dữ liệu có thể thay đổi theo nguồn dữ liệu hiện có.' : 'Features and data may vary depending on available sources.'}
        </p>
      </div>
    </section>
  )
...
function HomeMissionBanner({ vi, covers }: { vi: boolean; covers: Cover[] }) {
  ...
  return (
    <section className="pb-16 sm:pb-20">
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
        <div className="grid md:grid-cols-[0.9fr_1.1fr] gap-0 overflow-hidden rounded-2xl"
          style={{ background: 'var(--home-mission-bg)', border: '1px solid var(--home-mission-border)', boxShadow: 'var(--home-mission-shadow)' }}>
            ...
            {visualCovers.length > 0 ? (
              <div className="absolute inset-0 grid grid-cols-3 gap-3 p-4 sm:p-5 items-center">
                {visualCovers.map((cover, idx) => (
                  <div key={`${cover.id}-${idx}`} className="relative rounded-xl overflow-hidden h-full"
                    style={{ aspectRatio: '2 / 3', maxHeight: '100%', justifySelf: 'center', border: '1px solid var(--home-mission-border)', boxShadow: '0 12px 32px rgba(15, 23, 42, .16)', background: 'var(--home-mission-cover-fallback)' }}>
                    <div className="absolute inset-0 flex items-center justify-center" style={{ color: 'var(--foreground-muted)' }}>
                      <BookOpen className="w-8 h-8 opacity-40" />
                    </div>
                    {cover.cover_url ? (
                      <img src={cover.cover_url} alt="" loading="lazy" decoding="async" className="relative z-10 w-full h-full object-cover" onError={e => { e.currentTarget.style.display = 'none' }} />
                    ) : (
          ...
          <div className="relative p-6 sm:p-8 md:p-10 flex flex-col justify-center">
            <div className="absolute inset-0 opacity-70" style={{ background: 'var(--home-mission-glow)' }} />
            <div className="relative">
...
export default function HomeClient({ initialData }: { initialData: HomeData }) {
  ...
  const initialCarouselData = useMemo<Record<CarouselSection, CarouselItem[]>>(() => ({
    ...
    manga: data.recentManga.map((m: any) => ({ id: m.id, title: m.title, cover_url: data.mangaVolCovers[m.id] !== undefined ? data.mangaVolCovers[m.id] : m.cover_url, score: null, href: `/content/${m.id}` })),
  ...
  const [typeCounts] = useState<TypeCounts | null>(() => ({ anime: data.typeCounts[0] || 0, manga: data.typeCounts[1] || 0, novel: data.typeCounts[2] || 0 }))
  const [carouselData] = useState<Record<CarouselSection, CarouselItem[]>>(initialCarouselData)
  const [carouselReady] = useState(true)
    ...
    <div style={{ background: 'var(--background)' }}>
      <section className="relative flex flex-col overflow-hidden" style={{ minHeight: '100svh' }}>
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute inset-0 z-10" style={{ background: 'var(--hero-wall-overlay)' }} />
          ...
          <div className="absolute inset-0 items-start overflow-hidden" style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: '10px', padding: '0 4px' }}>
            {hasCols
              ? [
                ...
                ].map((col, i) => <CoverColumn key={i} covers={col.covers} speed={col.speed} offset={col.offset} delay={col.delay} />)
              : Array.from({ length: 8 }).map((_, i) => (
                  <div key={i} className="flex flex-col gap-2 w-full">
                    {Array.from({ length: 8 }).map((_, j) => (
                      <div key={j} className="rounded-lg flex-shrink-0 animate-pulse" style={{ aspectRatio: '2/3', background: 'rgba(99,102,241,0.07)' }} />
                    ))}
                  </div>
                ))}
          </div>
        ...
        <div className="relative z-20 flex-1 flex flex-col justify-center">
          <div className="max-w-7xl mx-auto w-full px-6 sm:px-10 lg:px-16 py-24">
            <p className="text-xs sm:text-sm font-semibold tracking-widest uppercase mb-5" style={{ color: '#818cf8', letterSpacing: '0.18em' }}>
              {vi ? 'Phân tích · Dữ liệu · Cộng đồng' : 'Analytics · Data · Community'}
            </p>
            <h1 className="font-black leading-none tracking-tight mb-6" style={{ fontSize: 'clamp(2.8rem, 7vw, 5.5rem)', color: 'var(--foreground)', maxWidth: '10ch' }}>
              {vi ? <>Khám phá<br /><span style={{ color: '#818cf8' }}>tựa tiếp</span><br />theo</> : <>Discover<br /><span style={{ color: '#818cf8' }}>your next</span><br />obsession</>}
            </h1>
            <p className="text-base sm:text-lg mb-10 max-w-sm leading-relaxed" style={{ color: 'var(--hero-secondary-text)', fontWeight: 500 }}>
              {vi ? 'Điểm số, xu hướng và thống kê sâu cho Anime, Manga & Light Novel.' : 'Scores, trends and deep stats for Anime, Manga & Light Novels.'}
            </p>
            <div className="flex items-center gap-3 flex-wrap">
              <Link href="/browse" className="group flex items-center gap-2 px-6 py-3.5 rounded-xl text-sm font-bold text-white transition-all duration-200 hover:brightness-110 hover:-translate-y-0.5" style={{ background: '#6366f1', boxShadow: '0 4px 20px rgba(99,102,241,0.35)' }}>
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-base sm:text-lg font-bold ml-1 transition-colors duration-300" style={{ color }}>{vi ? SECTION_CONFIG[activeSection].labelVI : SECTION_CONFIG[activeSection].label}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1.5 p-1 rounded-full" style={{ background: 'var(--glass-bg)', border: '1px solid var(--card-border)' }}>
                  {sections.map(s => (
                    <button key={s} onClick={() => { setAutoRotate(false); goToSection(s) }} className="px-3 py-1 rounded-full text-xs font-bold transition-all duration-200 whitespace-nowrap" style={activeSection === s ? { background: SECTION_CONFIG[s].color, color: '#fff', boxShadow: `0 2px 8px ${SECTION_CONFIG[s].color}44` } : { color: 'var(--foreground-secondary)' }}>
                ...
                {autoRotate && (
                  <div className="hidden sm:flex gap-1">
                    {sections.map(s => (
                      <div key={s} className="h-0.5 rounded-full transition-all duration-300" style={{ width: activeSection === s ? 20 : 6, background: activeSection === s ? SECTION_CONFIG[s].color : 'var(--card-border)' }} />
                    ))}
                  </div>
                )}

                <Link href={SECTION_CONFIG[activeSection].href} className="hidden sm:flex items-center gap-1 text-xs font-semibold transition-all hover:gap-2" style={{ color: 'var(--foreground-muted)' }}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-2 sm:gap-x-4 gap-y-8 sm:gap-y-10" style={{ opacity: transitioning ? 0 : 1, transition: 'opacity 0.22s ease' }} onMouseEnter={stopTop10Rotation} onPointerDown={stopTop10Rotation} onFocus={stopTop10Rotation}>
              {!carouselReady ? (
                ...
                <TopCard key={item.id} item={item} rank={i + 1} accentColor={color} scoreLabel={activeSection === 'novel' ? 'votes' : undefined} onInteract={stopTop10Rotation} />
              ))}
            </div>

            <div className="flex justify-center mt-8 sm:hidden">
              <Link href={SECTION_CONFIG[activeSection].href} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all hover:scale-105" style={{ background: `${color}18`, color, border: `1px solid ${color}33` }}>
                {vi ? 'Xem tất cả' : 'View all'}
